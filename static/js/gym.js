  ...
  forms.forEach((form) => {
    form
      .addEventListener("submit", function (e) {
        ...
        setTimeout(() => {
          // Fillimisht zbehim mesazhin e suksesit
          successMessage.style.opacity = "0";

          setTimeout(() => {
            successMessage.style.display = "none";

            // Resetojmë fushat e formës
            form.reset();

            // Shfaqim formën përsëri
            formContainer.style.display = "block";
            setTimeout(() => {
              formContainer.style.opacity = "1";
            }, 50);
          }, 400); // Koha e animacionit fade-out
        }, 5000); // 5 sekonda qëndron mesazhi i suksesit hapur
      })
      .catch((error) => {
        console.error("Gabim:", error);
        alert("Pati një problem me dërgimin.");
      });
  });
});
